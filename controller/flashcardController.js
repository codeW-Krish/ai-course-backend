    ...
    try {
        ...
        for (const chunk of chunks) {
            const snap = await flashcardsRef
                ...
                .get();
            snap.docs.forEach((doc) => allFlashcards.push({ id: doc.id, ...doc.data() }));
        }
        ...
        for (const card of allFlashcards) {
            ...
            if (!progDoc.exists) {
                ...
                const prog = progDoc.data();
                const nextReview = prog.next_review_at?.toDate
                    ? prog.next_review_at.toDate()
                    : new Date(prog.next_review_at);
